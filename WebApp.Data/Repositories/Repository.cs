...
namespace WebApp.Data.Repositories
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public async Task<IEnumerable<T>> GetAllSortedAsync<TKey>(Expression<Func<T, TKey>> orderBy, string orderByDirection = GeneralConstants.OrderBy.Ascending)
        {
        ...
        public async Task<IEnumerable<T>> GetFilteredSortedAsync<TKey>(Expression<Func<T, bool>> predicate, Expression<Func<T, TKey>> orderBy, string orderByDirection = GeneralConstants.OrderBy.Ascending, params Expression<Func<T, object>>[] includes)
        {
            var query = ApplyIncludes(_dbSet.AsNoTracking().Where(predicate), includes);
            return orderByDirection == GeneralConstants.OrderBy.Ascending
                ? await query.OrderBy(orderBy).ToListAsync()
