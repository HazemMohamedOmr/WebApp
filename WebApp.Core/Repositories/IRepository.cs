...
namespace WebApp.Core.Repositories
{
    public interface IRepository<T> where T : class
    {
        ...
        // Sorting
        Task<IEnumerable<T>> GetAllSortedAsync<TKey>(Expression<Func<T, TKey>> orderBy, string orderByDirection = GeneralConstants.OrderBy.Ascending);
        Task<IEnumerable<T>> GetFilteredSortedAsync<TKey>(Expression<Func<T, bool>> predicate, Expression<Func<T, TKey>> orderBy, string orderByDirection = GeneralConstants.OrderBy.Ascending, params Expression<Func<T, object>>[] includes);
